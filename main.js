...
methods.set('/posts.post', async ({response, searchParams, db}) => {
  ...
  const showId = result.getAutoIncrementValue();

  const showResult = await table.select(['id', 'content', 'likes', 'created'])
    .where('id = :id')
    .bind('id', showId)
    .execute();

  const data = showResult.fetchAll();
  const columns = showResult.getColumns();
  const post = data.map(map(columns));
  sendJSON(response, post[0]);
});
...
methods.set('/posts.like', async ({response, searchParams, db}) => {
  ...
  const table = await db.getTable('posts');
  const likes = await table.select('likes')
    .where('id = :id AND removed = :removed')
    .bind('id', id)
    .bind('removed', false)
    .execute();

  const dataLikes = likes.fetchAll();
  const columnLikes = likes.getColumns();
  const postLikes = dataLikes.map(map(columnLikes));
  ...
  const updateLikes = await table.update()
    .set('likes', postLikes[0].likes + 1)
    .where('id = :id')
    .bind('id', id)
    .execute();

  const updated = updateLikes.getAffectedItemsCount();
